...
or fragile links, so do not lean on it as a supplement.`,
      inputSchema: {
        ...
        court: z
          ...
          .optional()
          .describe(
            "Restrict to one court, given as its slug. Accepted values: 'uksc', 'ukpc', 'ewca/civ', 'ewca/crim', 'ewhc/kb', 'ewhc/ch', 'ewhc/comm', 'ewhc/fam', 'ewhc/pat', 'ewhc/ipec', 'ewhc/admin', 'ewhc/tcc', 'ewhc/costs', 'ewfc', 'ewcop', 'eat', 'ukut/iac', 'ukut/aac', 'ukut/tcc', 'ukut/lc', 'ukftt/tc', 'ukftt/grc'. Find Case Law covers England & Wales only — it does not host Scottish, Northern Irish, or Irish courts.",
          ),
        judge: z
          .string()
          .optional()
...
function registerCaseLawResources(server: McpServer, deps: Deps): void {
  server.registerResource(
    ...
    {
      description:
        "A TNA judgment's metadata — parties, judges, neutral citation, court, and dates — about 1,000 tokens. Read this first to orient yourself on a judgment. Example slugs: 'uksc/2024/12', 'ewca/civ/2023/450'.",
  ...
  server.registerResource(
    ...
    {
      description:
        "A navigation index of 'eId: first_line' rows, one per paragraph (roughly 4,000 tokens for a typical Supreme Court decision). Use it to learn the paragraph identifiers, then open individual paragraphs via judgment://{slug}/para/{eId}. To find paragraphs by content instead, use the case_law_grep_judgment tool.",
      mimeType: "text/plain",
    },
    async (uri, variables) => {
      const slug = String(variables.slug);
  ...
  server.registerResource(
    ...
    {
      description:
        "One <paragraph> element identified by its LegalDocML eId (for example 'para_12'), with any sub-paragraphs inside it. Usually 400-1,700 tokens. Check the index resource for the available eIds.",
      mimeType: "application/xml",
    },
    async (uri, variables) => {
      const slug = String(variables.slug);
