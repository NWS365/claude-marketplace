 ...
 */
async function mintAccessToken(base: string, key: string, secret: string): Promise<string | undefined> {
...
export function registerEpoOpsTools(server: McpServer, deps: Deps): void {
  // -------------------------------------------------------------------------
    ...
    "epo_ops_search_patents",
    {
      ...
      inputSchema: {
        query: z
          ...
          .max(250)
          .describe(
            "Free text (matched against title/abstract) or a CQL expression containing '=', e.g. 'graphene battery' or 'pa=\"acme\"'"
          ),
        limit: z
          .number()
          .int()
    ...
    async (args) => {
      ...
      if (!key || !secret) {
        return toolError("configuration", {
          ...
          attempted,
          description:
            "EPO OPS credentials are not set. Provide EPO_OPS_CONSUMER_KEY and EPO_OPS_CONSUMER_SECRET (free from developers.epo.org).",
        });
      }
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      try {
        ...
        const url = `${base}/rest-services/published-data/publication/epodoc/${encodeURIComponent(
