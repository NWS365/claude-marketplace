...
import type { GazetteNotice, GazetteSearchResult } from "./models.js";

export function registerGazetteTools(server: McpServer, deps: Deps): void {
  // ---------------------------------------------------------- search_notices
  server.registerTool(
...
statutory source.`,
      inputSchema: {
        query: z
          ...
          .max(300)
          .describe(
            "Free-text query, matched across notice content — for example a company name, a person's name, or 'members voluntary liquidation'.",
          ),
        edition: z
          .enum(["London", "Edinburgh", "Belfast"])
          .optional()
          .describe(
            "Restrict to one Gazette edition. London covers England and Wales, Edinburgh covers Scotland, and Belfast covers Northern Ireland. Omit to search all three.",
          ),
        notice_type: z
          .string()
          .optional()
          .describe(
            "Restrict to one or more notice-type codes, each a 4-digit code. Join several with '+' (for example '2450+2451'). Leave unset to include every notice type.",
          ),
        category_code: z
          .string()
          .optional()
          .describe(
            "Restrict to a Gazette notice category, given as its 2-digit code (for example '24' for insolvency notices).",
          ),
        from_date: z
          .string()
          .optional()
    ...
    async (args) => {
      ...
      qp.set("text", query);
      if (edition) qp.set("edition", edition);
      if (notice_type) qp.set("noticetype", notice_type);
      if (category_code) qp.set("categorycode", category_code);
      if (from_date) qp.set("start-publish-date", from_date);
      if (to_date) qp.set("end-publish-date", to_date);
      try {
        const f = assertOk(
          await deps.xmlGet(`${GAZETTE_BASE}/all-notices/notice/data.feed?${qp.toString()}`, {
            cacheTtl: TTL.HOUR,
          }),
        );
        const { total, results } = parseGazetteFeed(f.text, limit);
        const payload: GazetteSearchResult = { query, total, results };
...
substitute a general web search for it.`,
    ...
    async (args) => {
      ...
      try {
        const f = assertOk(
          await deps.xmlGet(`${GAZETTE_BASE}/notice/${encodeURIComponent(id)}`, { cacheTtl: TTL.HOUR }),
        );
        const parsed = parseGazetteNotice(f.text);
        const notice: GazetteNotice = {
          id,
