 ...
 * (xmldom + xpath).
 */
import { parseXml, nsSelector, textOf, attr, type XmlNode } from "../../shared/xml.js";
import type { GazetteNoticeSummary } from "./models.js";
 ...
 */
function bestLink(entry: XmlNode, ns: ReturnType<typeof nsSelector>): string | null {
  ...
  for (const link of links) {
    ...
    if (rel === "alternate" && alternate === null) alternate = href;
    else if (rel === "self" && self === null) self = href;
  }
  return alternate ?? self ?? first;
}

/** Turn a single Atom <entry> node into a notice summary. */
function extractEntry(entry: XmlNode, ns: ReturnType<typeof nsSelector>): GazetteNoticeSummary {
 ...
 */
export function parseGazetteFeed(
  xmlText: string,
  limit: number = 10,
): { total: number; results: GazetteNoticeSummary[] } {
  try {
 ...
 */
export function parseGazetteNotice(xmlText: string): GazetteNoticeSummary | null {
  try {
    const doc = parseXml(xmlText);
