...
/**
 * Stateless parsing and helper routines for the parliament module. Everything
 * here is side-effect free apart from populateVotesIds, which accepts Deps and
 * resolves the Lords/Commons Votes API identifiers over the network.
 */
...
export const INTEREST_CATEGORIES: Record<string, number> = {
  employment: 12,
  employment_adhoc: 1,
 ...
 */
export function safeInt(value: any, def = 0): number {
...
function strictInt(value: any): number | null {
  if (typeof value === "number" && Number.isFinite(value)) return Math.trunc(value);
  if (typeof value === "string") {
    const t = value.trim();
    if (/^[+-]?\d+$/.test(t)) return parseInt(t, 10);
...
export function pyIntOr0(x: any): number {
 ...
 */
export function isoDate(raw: any): string {
  const base = raw || "1970-01-01";
  const s = String(base).slice(0, 10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(s)) throw new Error(`invalid isoformat string: ${s}`);
  return s;
}
 ...
 */
export function formatHttpError(e: unknown): string {
  if (e instanceof UpstreamHttpError) {
    const status = e.status;
    if (status === 404) return "Error 404: Resource not found — check the URI or identifier is correct.";
    if (status === 403) return `Error 403: Access denied by upstream API. URL: ${e.url}`;
    if (status === 429) return "Error 429: Rate limit hit — upstream API is throttling. Try again shortly.";
    if (status === 503) return "Error 503: Upstream service unavailable — try again later.";
...
export function mostCommon(counter: Map<string, number>): Array<[string, number]> {
  return [...counter.entries()].sort((a, b) => b[1] - a[1]);
}

/** Coerce an upstream House value to Commons or Lords, defaulting to Commons. */
export function normHouse(raw: any): House {
  const h = raw || "Commons";
...
export function hansardContributionUrl(
  house: string,
  sittingDate: string,
  debateExtId: string,
  ...
  return (
    `https://hansard.parliament.uk/${houseSeg}/${sittingDate}` +
...
export function extractColumnNumbers(html: string): number[] {
  if (!html) return [];
  const out: number[] = [];
  SPAN_RE.lastIndex = 0;
  ...
  while ((m = SPAN_RE.exec(html)) !== null) {
    const attrs = m[1] ?? "";
    if (!IS_COLUMN_MARKER_RE.test(attrs)) continue;
    const colM = COL_NUM_ATTR_RE.exec(attrs);
    if (colM) out.push(parseInt(colM[1]!, 10));
  }
  return out;
}

/** Compute carry-forward column numbers, indexed by item position. */
export function assignColumns(items: any[]): Array<[number | null, number | null]> {
  const out: Array<[number | null, number | null]> = [];
  let currentColumn: number | null = null;
  for (let idx = 0; idx < items.length; idx++) {
    const markers = extractColumnNumbers((items[idx]?.Value as string) || "");
    const colStart = markers.length ? markers[0]! : currentColumn;
    if (markers.length) currentColumn = markers[markers.length - 1]!;
...
export function itemIsContribution(item: any): boolean {
  if ((item?.ItemType || "") !== "Contribution") return false;
  const hrs = item?.HRSTag || "";
  if (hrs === "hs_ColumnNumber") return false;
  return Boolean(item?.Value);
}

const HANSARD_SOURCE_LABELS: Record<number, string> = {
  1: "RollingHansard",
  2: "DailyHansard",
  3: "BoundVolume",
  4: "Historic",
...
export function hansardSourceLabel(code: any): string | null {
...
export function parseHansardContributions(
  data: any,
  textMode: "preview" | "full" = "preview",
  maxTextChars = 3000
): HansardContribution[] {
  const contributions: HansardContribution[] = [];
  for (const item of data?.Contributions ?? []) {
    try {
      const attr: string = item.AttributedTo || item.MemberName || "";
      const name: string = item.MemberName || "Unknown";
      ...
      const house: string = item.House || "Commons";
      const debateExtId: string = item.DebateSectionExtId || "";
      const contributionExtId: string = item.ContributionExtId || "";
      const debateTitle = (item.DebateSection || item.DebateSectionName || "Unknown").trim() || "Unknown";
      const url =
        debateExtId && contributionExtId
...
export function parseDebateItemAsContribution(
  item: any,
  overview: any,
  columnAssignment: Array<[number | null, number | null]>,
  itemIndex: number,
  maxTextChars = 3000
): HansardContribution | null {
  try {
    const attr: string = (item.AttributedTo || "").trim();
    if (!attr) return null;
    let name = attr;
    let party: string | null = null;
    ...
    const debateExtId: string = overview.ExtId || "";
    const debateId = safeInt(overview.Id, 0);
    const debateTitle = (overview.Title || "Unknown").trim() || "Unknown";
    const chamberSection: string = overview.Location || `${house} Chamber`;
    const contributionExtId: string = item.ExternalId || "";
...
export function computeSearchFacets(contributions: HansardContribution[]): {
...
export function parseTopDebatesPreview(payload: any): TopDebate[] {
  const out: TopDebate[] = [];
...
export function parseDivisionMatch(item: any): DivisionMatchLite | null {
  try {
    const extId: string = item.ExternalId || "";
    if (!extId) return null;
    const sittingDate = isoDate(item.Date);
    const house = normHouse(item.House);
    const timeRaw = item.Time;
    let timeClean: string | null = null;
...
export function parseTopDivisionsPreview(payload: any): DivisionMatchLite[] {
  const out: DivisionMatchLite[] = [];
 ...
 * (date, house) group.
 */
export async function populateVotesIds(deps: Deps, divisions: DivisionMatchLite[]): Promise<void> {
  if (!divisions.length) return;
  const groups = new Map<string, DivisionMatchLite[]>();
  ...
  for (const [key, group] of groups) {
    ...
    for (const entry of payload) {
      ...
      if (n === null || n === undefined || divId === null || divId === undefined) continue;
