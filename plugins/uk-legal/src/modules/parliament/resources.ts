...
import { assertOk, jsonOf, type Fetched } from "../../shared/http.js";
...
function jsonContents(uri: URL, obj: unknown, indent: number): ReadResourceResult {
  return {
    contents: [
      {
        uri: uri.href,
...
async function fetchDebate(deps: Deps, debateExtId: string): Promise<any> {
  const f: Fetched = assertOk(await deps.jsonGet(`${HANSARD_API}/debates/Debate/${debateExtId}.json`, { cacheTtl: TTL.HOUR }));
  return jsonOf<any>(f);
}

async function fetchBiography(deps: Deps, memberId: number): Promise<any> {
  const f: Fetched = assertOk(await deps.jsonGet(`${MEMBERS_BASE}/Members/${memberId}/Biography`, { cacheTtl: TTL.HOUR }));
  return jsonOf<any>(f).value ?? {};
}

export function registerParliamentResources(server: McpServer, deps: Deps): void {
  // -------------------------------------------------------------------------
  // hansard://debate/{debate_ext_id}/header
  // -------------------------------------------------------------------------
  server.registerResource(
    "Hansard Debate — header + ordered contribution index",
    new ResourceTemplate("hansard://debate/{debate_ext_id}/header", { list: undefined }),
    {
      description:
        "An overview of the debate (title, date, house, location, volume) " +
  ...
  // -------------------------------------------------------------------------
  server.registerResource(
    "Hansard Contribution — single contribution full text",
    new ResourceTemplate("hansard://debate/{debate_ext_id}/contribution/{contribution_ext_id}", {
      list: undefined,
    }),
    {
      description:
        "The complete text of one contribution alongside its member metadata " +
    ...
    async (uri, variables): Promise<ReadResourceResult> => {
      ...
      const htmlValue: string = match.Value || "";
  ...
  // -------------------------------------------------------------------------
  server.registerResource(
    "UK Parliament Member — biography",
    new ResourceTemplate("hansard://member/{member_id}/biography", { list: undefined }),
    {
      description:
        "The complete biography from the Members API: constituency " +
