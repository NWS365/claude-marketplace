/**
 * Constants and helpers that turn the upstream bills JSON into the module's
 * typed shapes.
 *
 * Every string field is whitespace-trimmed so the resulting objects carry
 * clean values.
 */
import type { BillDetail, BillSponsor, BillStage, BillSummary } from "./models.js";

export const BILLS_BASE = "https://bills-api.parliament.uk/api/v1";

export const HOUSE_MAP: Record<string, number> = { Commons: 1, Lords: 2 };
...
function parseHouse(v: any): string | null {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function parseBillSummary(item: any): BillSummary {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function parseBillDetail(data: any, maxSummaryChars: number): BillDetail {
  ...
  for (const s of rawSponsors) {
    ...
    let nameVal: unknown;
    if ("name" in member) nameVal = member.name;
    else if (s && typeof s === "object" && "name" in s) nameVal = s.name;
    else nameVal = "Unknown";
    sponsors.push({
  ...
  if (csd && typeof csd === "object" && !Array.isArray(csd)) {
    ...
    const stageNameVal: unknown = desc || stageName;
    currentStageName = stageNameVal == null ? null : String(stageNameVal);

    let sittingDate: string | null = null;
    ...
    if (Array.isArray(sittings) && sittings.length > 0) {
      ...
      if (dateStr) {
        ...
        if (isValidIsoDate(iso)) sittingDate = iso;
      }
    }
  ...
  const royalAssentDate: string | null = null;

  const rawSummary = data?.summary;
  let summary: string | null;
  let summaryTruncated: boolean;
  let summaryOriginalLength: number;
