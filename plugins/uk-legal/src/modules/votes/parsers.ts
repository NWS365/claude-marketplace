/**
 * Endpoint builders and side-effect-free parsers for the votes module.
 *
 * Because the Commons and Lords votes APIs disagree on JSON layout — PascalCase
 * against camelCase keys, and differently named count fields — these helpers
 * fold both responses down to the common model types.
 */
import type { DivisionSummary, Voter } from "./models.js";
...
export function searchUrl(house: string): string {
  if (house === "Lords") return `${LORDS_VOTES_BASE}/data/Divisions/search`;
  return `${COMMONS_VOTES_BASE}/data/divisions.json/search`;
}

export function detailUrl(house: string, divisionId: number): string {
  if (house === "Lords") return `${LORDS_VOTES_BASE}/data/Divisions/${divisionId}`;
...
export function firstOf(obj: Dict, keys: string[], def: unknown): unknown {
  for (const k of keys) {
...
function strip(v: unknown, def: string): string {
...
export function parseCommonsSummary(item: Dict): DivisionSummary {
...
export function parseLordsSummary(item: Dict): DivisionSummary {
...
export function parseVoters(voterList: unknown[]): Voter[] {
  const voters: Voter[] = [];
...
export function detailDate(v: unknown): string {
  return toDateStr(v);
}

/** Title helper for the detail path (exported for tools.ts; trims surrounding whitespace). */
export function detailTitle(v: unknown): string {
