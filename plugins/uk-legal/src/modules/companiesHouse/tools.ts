...
export function registerCompaniesHouseTools(server: McpServer, deps: Deps): void {
  // -------------------------------------------------------------------------
    ...
    async (args) => {
      ...
      if (!apiKey) {
        return toolError("configuration", {
          ...
          attempted,
          description:
            "Companies House API key is not set. Provide the COMPANIES_HOUSE_API_KEY environment variable (free from developer.company-information.service.gov.uk).",
        });
      }
      try {
        ...
        const f = assertOk(
          await deps.jsonGet(url, { headers: { Authorization: basicAuthHeader(apiKey) }, cacheTtl: TTL.HOUR })
        );
