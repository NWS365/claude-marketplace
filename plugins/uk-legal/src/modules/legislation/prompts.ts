...
export function registerPrompts(server: McpServer, _deps: Deps): void {
  server.registerPrompt(
    "legislation_summarise_act",
    {
      ...
      description:
        "Write a summary of a UK Act of Parliament or Statutory Instrument.\n\n" +
    ...
    (args) => {
      ...
      const text =
        `Act as a UK legal analyst and summarise ${type}/${year}/${number}.\n\n` +
  ...
  server.registerPrompt(
    "legislation_compare_legislation",
    {
      ...
      description:
        "Set two pieces of UK legislation side by side on a chosen topic.\n\n" +
    ...
    (args) => {
      ...
      const text =
        `Compare ${type1}/${year1}/${number1} with ${type2}/${year2}/${number2}, ` +
