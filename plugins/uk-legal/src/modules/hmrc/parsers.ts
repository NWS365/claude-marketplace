/**
 * Stateless lookup and response-shaping helpers for the hmrc module.
 *
 * Nothing here touches the network: the handlers in tools.ts do the fetching,
 * and these functions turn raw responses into the snake_case wire objects
 ...
 */
const VAT_LOOKUP: ReadonlyArray<readonly [string, VatRateCategory, number, string]> = [
 ...
 * any trimming.
 */
export function lookupVat(commodityCode: string): VATRate {
  ...
  return {
    ...
    effective_from: EFFECTIVE_DATE,
    notes:
      `No specific exemption found for '${commodityCode}'. ` +
 ...
 */
export function isoDateOrNull(ts: unknown): string | null {
...
export function parseMtdStatus(vrn: string, raw: unknown): MTDStatus {
  const data = (raw ?? {}) as {
    obligations?: unknown;
  ...
  if (obligations.length > 0) {
    const first = (obligations[0] ?? {}) as { start?: string | null };
    if (first.start) effectiveDate = first.start;
  }
  return {
    vrn,
...
export function parseGuidanceResults(query: string, raw: unknown): HMRCGuidanceSearchResult {
  const data = (raw ?? {}) as { results?: unknown };
  const items: unknown[] = Array.isArray(data.results) ? data.results : [];
  const results: HMRCGuidanceResult[] = [];
