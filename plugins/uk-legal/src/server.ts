...
const ABOUT = {
  ...
  llm_posture:
    "this server runs no LLM of its own. All tool responses come directly from the named APIs, EXCEPT citations_parse with disambiguate=true (off by default), which asks the connected client's own model to resolve an ambiguous court division via MCP sampling.",
...
export function buildServer(): McpServer {
  ...
  server.registerResource(
    "About this server",
    "server://about",
    { title: "About this server", description: "Provenance, upstream APIs, and operational posture.", mimeType: "application/json" },
    async (uri) => ({ contents: [{ uri: uri.href, mimeType: "application/json", text: JSON.stringify(ABOUT, null, 2) }] })
  );
