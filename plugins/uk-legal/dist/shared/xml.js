...
export function parseXml(input) {
    ...
    if (DTD_RE.test(text)) {
        throw new Error("XML payload contains DTD/ENTITY declaration — rejected for safety. " +
    ...
    if (errors.length)
        throw new Error(`XML parse error: ${errors[0]}`);
    return doc;
}
