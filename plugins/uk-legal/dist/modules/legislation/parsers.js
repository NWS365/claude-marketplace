 ...
 */
import { parseXml, parseHtml, nsSelector, select, textOf, attr } from "../../shared/xml.js";
...
function directText(el) {
    ...
    if (first && (first.nodeType === 3 || first.nodeType === 4))
...
function directChild(parent, nsUri, local) {
    for (let c = parent?.firstChild; c; c = c.nextSibling) {
        if (c.nodeType === 1 && c.localName === local && c.namespaceURI === nsUri)
            return c;
    }
    return null;
}
...
export function normaliseSectionId(section) {
...
export function parseSearchAtom(xmlText) {
...
function extentCodesToNames(codeString) {
    ...
    for (const raw of codeString.split("+")) {
        const code = raw.trim();
        if (Object.hasOwn(EXTENT_CODE_MAP, code))
            names.push(EXTENT_CODE_MAP[code]);
    }
    return names;
}
/** The candidate chain that the RestrictExtent and RestrictStartDate lookups climb. */
function collectCandidates(sectionEl, root) {
    const candidates = [];
    if (sectionEl) {
        candidates.push(sectionEl);
        for (const a of ancestors(sectionEl))
            candidates.push(a);
    }
...
function findRestrictExtent(sectionEl, root) {
...
function findRestrictStartDate(sectionEl, root) {
...
function sectionIsRepealed(sectionEl, sel) {
    if (!sectionEl)
        return false;
    let title = directChild(sectionEl, LEG_NS, "Title");
    if (!title)
        title = sel.first(".//leg:Title", sectionEl);
    if (!title)
        return false;
    return sel.first(".//leg:Repeal", title) !== null;
}
export function parseClmlSection(xmlText, section, maxChars) {
    const root = parseXml(xmlText).documentElement;
    ...
    let sectionEl = sel.first(`.//leg:P1group[@id='section-${section}']`, root);
    if (!sectionEl) {
    ...
    if (!extentCodes) {
        ...
        if (legacyExtentEl)
            extentCodes = attr(legacyExtentEl, "Value") ?? "";
    }
    ...
    if (rsd)
        versionDate = parseIsoDate(rsd);
    if (versionDate === null) {
        const dateEl = sel.first(".//ukm:EnactmentDate", root);
        if (dateEl)
            versionDate = parseIsoDate(attr(dateEl, "Date") ?? "");
    }
    ...
    if (sectionEl) {
        ...
        if (!titleEl)
            titleEl = sel.first(".//leg:Title", sectionEl);
        if (titleEl) {
...
export function parseHtmlSection(htmlText, section, maxChars, warning) {
    const doc = parseHtml(htmlText);
    const root = doc.documentElement;
    const candidates = select("//*[@id='content'] | //*[@id='viewLegSnippet'] | //*[@class='LegSnippet'] | " +
        "//*[local-name()='main'] | //*[local-name()='article'] | //*[local-name()='body']", doc);
    const container = candidates.length ? candidates[0] : root;
    // Strip out the navigation and page chrome that would otherwise leak into the section text.
    const noisy = select(".//*[local-name()='script'] | .//*[local-name()='style'] | .//*[local-name()='nav'] | " +
        ".//*[local-name()='footer'] | .//*[local-name()='header'] | .//*[local-name()='form']", container);
    ...
    const content = truncated ? rawContent.slice(0, maxChars) + " …[truncated]" : rawContent;
    let title = `Section ${section}`;
    const heading = select(".//*[local-name()='h1']/text() | .//*[local-name()='h2']/text() | .//*[local-name()='h3']/text()", container);
    if (heading.length) {
        const headingText = heading[0].nodeValue ?? "";
        title = headingText
            .split(/\s+/)
            .filter((s) => s.length > 0)
            .join(" ");
...
export function parseTocXml(xmlText) {
    const root = parseXml(xmlText).documentElement;
    const items = [];
