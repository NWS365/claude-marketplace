...
export function registerEurlexTools(server, deps) {
    // -------------------------------------------------------------------------
    ...
    server.registerTool("eurlex_search", {
        ...
        inputSchema: {
            query: z
                ...
                .min(2)
                .max(200)
                .describe("Words to match within the English title of an EU instrument, e.g. 'data protection', 'market abuse', 'medical devices'. Matched case-insensitively as a substring."),
            limit: z
                .number()
                .int()
                .gte(1)
...
LIMIT ${args.limit}`;
    ...
    // -------------------------------------------------------------------------
    server.registerTool("eurlex_get_document", {
        ...
        inputSchema: {
            celex: z
                ...
                .min(3)
                .max(40)
                .describe("A CELEX identifier — the EU's canonical document number, e.g. '32016R0679' (GDPR) or '32014L0065' (MiFID II)."),
        },
