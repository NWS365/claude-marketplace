...
export function registerCommitteesTools(server, deps) {
    // --- committees_search_committees ------------------------------------------
    server.registerTool("committees_search_committees", {
...
the oral and written evidence submitted to it.`,
        inputSchema: {
            query: z
                .string()
                .max(300)
                .describe("Text to match against committee names, such as 'defence' or 'treasury'. Matching is performed locally on the returned names. Leave unset to enumerate every committee.")
                .optional(),
            ...
            limit: z
                ...
                .gte(1)
                .lte(500)
                .describe("Upper bound on how many committees come back. The default of 100 is more than enough for every UK select committee sitting today; raise it only when reaching back through historical records.")
                .default(100),
        },
        ...
        try {
            ...
            if (active_only)
                qp.set("CommitteeStatus", "Current");
            if (house)
                qp.set("House", String(HOUSE_MAP[house]));
    ...
    server.registerTool("committees_get_committee", {
...
subjects.`,
    ...
    server.registerTool("committees_search_evidence", {
...
This is the definitive record of evidence given to parliamentary committees.`,
        inputSchema: {
            ...
            offset: z
                ...
                .gte(0)
                .lte(2000)
                .describe("How many evidence items to pass over before this page begins. Starts at 0. To advance, call again with offset=offset+returned while has_more remains true.")
                .default(0),
            limit: z
                .number()
                .int()
                .gte(1)
                .lte(100)
                .describe("Cap on evidence items per page, defaulting to 20. With evidence_type='both', that budget is divided between oral and written submissions, roughly half to each.")
                .default(20),
            max_title_chars: z
                .number()
                .int()
                .gte(50)
                .lte(2000)
                .describe("Length limit applied to each item's title text. The default of 300 keeps lengthy inquiry titles from flooding the context; push it to 1000 or more only when the whole title matters.")
                .default(300),
        },
        ...
        const capTitle = (t) => t.length > max_title_chars ? t.slice(0, max_title_chars) + " …[truncated]" : t;
        const fetchEvidence = async (path, skip, take, attemptedType, mapFn) => {
            const qp = new URLSearchParams();
            ...
            try {
                ...
                throw new EvidenceFetchError(err, `committees_search_evidence(committee_id=${committee_id}, evidence_type='${attemptedType}')`);
            }
            const items = extractItems(jsonOf(f));
            const results = [];
            for (const raw of items) {
                if (isObj(raw))
                    results.push(mapFn(raw, capTitle));
            }
            return [results, items.length];
        };
        const fetchOral = (skip, take) => fetchEvidence("OralEvidence", skip, take, "oral", mapOral);
        const fetchWritten = (skip, take) => fetchEvidence("WrittenEvidence", skip, take, "written", mapWritten);
        try {
            let evidence = [];
            let has_more = false;
            ...
            if (err instanceof EvidenceFetchError)
                return toolErrorFromException(err.underlying, err.attempted);
