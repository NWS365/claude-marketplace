...
export function registerCitationsTools(server, deps) {
    server.registerTool("citations_parse", {
        ...
        try {
            ...
            return toolErrorFromException(err, `citations_parse(text=<${text.length} chars>, disambiguate=${quoteArg(disambiguate)})`);
        }
    });
    server.registerTool("citations_resolve", {
        ...
        try {
            ...
            if (allFound.length === 0) {
                throw new Error(`Could not recognise an OSCOLA citation in '${citation}'. ` +
        ...
        try {
            if (confidence === 0.0) {
                return jsonResult({
                    ...
                    detail: "Cannot format at confidence 0.0: citations_resolve found no live " +
            ...
            if (citation_type === "neutral" && resolved_url == null) {
                return jsonResult({
                    status: "upstream_bad_request",
                    detail: "Cannot format: this neutral citation has no resolved_url because " +
