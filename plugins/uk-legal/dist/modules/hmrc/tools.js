...
export function registerHmrcTools(server, deps) {
    ...
    // -------------------------------------------------------------------------
    server.registerTool("hmrc_get_vat_rate", {
        ...
        inputSchema: {
            commodity_code: z
                ...
                .min(2)
                .max(200)
                .describe("Either a commodity code or an everyday-language description, e.g. 'food', 'domestic fuel', 'software', 'financial services', 'new build residential'"),
        },
    ...
    // -------------------------------------------------------------------------
    server.registerTool("hmrc_check_mtd_status", {
        ...
        inputSchema: {
            vrn: z
                ...
                .min(9)
                .max(12)
                .describe("A 9-digit VAT Registration Number, e.g. '123456789'. A leading GB prefix is allowed and is removed for you."),
        },
        ...
        if (!clientId || !clientSecret) {
            return toolError("configuration", {
                ...
                description: "HMRC OAuth credentials are not set. Provide the HMRC_CLIENT_ID and HMRC_CLIENT_SECRET environment variables.",
            });
        }
        ...
        try {
            ...
            const f = assertOk(await deps.jsonGet(obligationsUrl, { headers: { Authorization: `Bearer ${accessToken}` } }));
            const data = jsonOf(f);
    ...
    // -------------------------------------------------------------------------
    server.registerTool("hmrc_search_guidance", {
