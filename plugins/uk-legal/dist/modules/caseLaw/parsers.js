...
import { parseXml, nsSelector, textOf, serialize, attr } from "../../shared/xml.js";
...
function firstLine(el, maxLen = FIRST_LINE_MAX) {
...
export function extractIndex(xmlText) {
    ...
    if (nativeEids.length) {
        for (const p of nativeEids)
            rows.push(`${attr(p, "eId")}: ${firstLine(p)}`);
    }
    else {
...
export function extractHeader(xmlText) {
    ...
    if (!header)
        throw new Error("No <header> element in this judgment");
    return serialize(header);
}
/** Serialise one `<paragraph>` element back to XML. */
export function extractParagraph(xmlText, eId) {
    const root = parseXml(xmlText);
    const akn = nsSelector(LEGALDOCML_NS);
    ...
    if (!el)
        throw new Error(`No paragraph with eId='${eId}'`);
    return serialize(el);
}
 ...
 */
export function grepParagraphs(xmlText, pattern, caseInsensitive = true, maxHits = 25) {
    ...
    for (let i = 0; i < allParas.length; i++) {
        ...
        let eId = attr(p, "eId");
        if (hasNative && !eId)
            continue; // modern judgments: pass over nested sub-items lacking an eId
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            if (ncnIdentEl) {
                ...
                if (ncnText)
                    identifiers.push({ type: "ukncn", value: ncnText, slug });
            }
            ...
            if (publishedEl == null)
                throw new Error("missing atom:published");
            const publishedText = (textOf(publishedEl) || "1970-01-01T00:00:00Z").trim();
            if (updatedEl == null)
                throw new Error("missing atom:updated");
