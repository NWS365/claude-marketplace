...
import { HANSARD_API, INTEREST_CATEGORIES, INTERESTS_BASE, MEMBERS_BASE, PETITIONS_BASE, assignColumns, computeSearchFacets, hansardSourceLabel, isoDate, itemIsContribution, mostCommon, normHouse, parseDebateItemAsContribution, parseDivisionMatch, parseHansardContributions, parseTopDebatesPreview, parseTopDivisionsPreview, populateVotesIds, pyIntOr0, quoteArg, safeInt, } from "./parsers.js";
export function registerParliamentTools(server, deps) {
    // -------------------------------------------------------------------------
    // parliament_search_hansard
    // -------------------------------------------------------------------------
    server.registerTool("parliament_search_hansard", {
...
web search or recalled training data.`,
        inputSchema: {
            ...
            member_id: z
                ...
                .int()
                .gte(1)
                .describe("Narrow the results to one member's contributions. Give the integer Members API ID here (turn a name into an ID with parliament_find_member). An earlier `member` field took a name string, but /search.json quietly dropped it — the specification calls for `memberId`.")
                .optional(),
            text_mode: z
                .enum(["preview", "full"])
                .default("preview")
                .describe("Pick 'preview' for the upstream snippet of roughly 250 characters — quick and light on context. Pick 'full' for ContributionTextFull, which is still limited to 3000 characters. To obtain the entire contribution with no cap, read the resource hansard://debate/{debate_ext_id}/contribution/{contribution_ext_id}."),
            contribution_type: z
                .enum(["Spoken", "Written", "Corrections"])
                .default("Spoken")
                .describe("Selects which Hansard section is paginated. 'Spoken' covers chamber and Westminster Hall debates — the default, and normally what a lawyer means. 'Written' covers written answers and statements. 'Corrections' covers published corrections to the record. The corpus envelope (total_debates, total_divisions, and so on) does not depend on this setting and is filled in regardless."),
            offset: z
                .number()
                .int()
                .gte(0)
                .lte(5000)
                .default(0)
                .describe("How many contributions to bypass before the returned page begins. Defaults to 0. To page onward, call again with offset raised by the number just returned; has_more indicates whether further results exist."),
            limit: z
                .number()
                .int()
                .gte(1)
                .lte(100)
                .default(20)
                .describe("Upper bound on contributions per call, from 1 to 100. Defaults to 20. Use offset to walk through further pages; the full corpus size sits in total_corpus on the response."),
        },
        ...
        try {
            ...
            if (from_date)
                params.set("startDate", from_date);
            if (to_date)
                params.set("endDate", to_date);
            if (house !== "both")
            ...
            assertOk(contribsF);
            assertOk(envelopeF);
            const contribsPayload = jsonOf(contribsF);
            const payload = jsonOf(envelopeF);
            const contributions = parseHansardContributions({ Contributions: contribsPayload.Results ?? [] }, text_mode);
    ...
    // -------------------------------------------------------------------------
    server.registerTool("parliament_policy_position_summary", {
...
Use this for corpus-level signals across UK Hansard.`,
        inputSchema: {
            ...
            max_debates_scanned: z
                ...
                .lte(2000)
                .default(200)
                .describe("Ceiling on how many debates are drawn from /search/Debates.json when building the facets. The default of 200 costs at most 4 upstream calls (50 per page). Push it to 2000 (up to 40 calls) for a thorough sweep of a heavily debated topic. Hansard's rate limit is 1000 req/5min."),
        },
        ...
        try {
            ...
            if (from_date)
                envParams.set("startDate", from_date);
            if (to_date)
                envParams.set("endDate", to_date);
            if (house !== "both")
                envParams.set("house", house);
            const envF = assertOk(await deps.jsonGet(`${HANSARD_API}/search.json?${envParams.toString()}`, { cacheTtl: TTL.HOUR }));
            const envelope = jsonOf(envF);
            const allDebates = [];
            const pageSize = 50;
            let skip = 0;
            ...
            const houseCounter = new Map();
            const sectionCounter = new Map();
            const yearCounter = new Map();
            const ymCounter = new Map();
            const topDebateModels = [];
            for (const d of allDebates) {
                ...
                const section = d.DebateSection || houseVal;
                ...
                const extId = d.DebateSectionExtId || "";
    ...
    // -------------------------------------------------------------------------
    server.registerTool("parliament_find_member", {
...
anti-bypass note shows why).`,
        ...
        try {
            ...
            const result = { query: name, total: members.length, members };
            return jsonResult(result);
        }
        catch (err) {
    ...
    // -------------------------------------------------------------------------
    server.registerTool("parliament_member_debates", {
...
The text field of each contribution is truncated at 3000 characters.`,
        inputSchema: {
            ...
            offset: z
                ...
                .lte(2000)
                .default(0)
                .describe("Count of contributions to pass over before this page starts. Defaults to 0. While has_more stays true, call again with offset increased by the number returned."),
            limit: z
                .number()
                .int()
                .gte(1)
    ...
    // -------------------------------------------------------------------------
    server.registerTool("parliament_member_interests", {
...
served from the Members API; a web search only turns up stale snapshots.`,
        inputSchema: {
            ...
            category: z
                ...
                .describe("Restrict results to one interest category. Frequently used ones: 'donations' (donations and support), 'gifts_uk' (UK gifts and hospitality), 'employment' (employment and earnings), 'land' (land and property), 'shareholdings', and 'overseas_visits'. Leave it unset to include every category.")
                .optional(),
            offset: z
                .number()
                .int()
                ...
                .lte(500)
                .default(0)
                .describe("How many interests to skip before this page begins. Use 0 for the first page. To work through members with many entries (100 or more), call again with offset raised by the number returned whenever the last response set has_more=true."),
            limit: z
                .number()
                .int()
                .gte(1)
                .lte(20)
                .default(20)
                .describe("Interests per call. The upstream interests-api.parliament.uk enforces a firm ceiling of 20 (confirmed live 2026-05-29: Take=100 still yields 20). Page through members with many entries using offset; the overall count appears in totalResults on the response."),
            max_description_chars: z
                ...
                .lte(5000)
                .default(500)
                .describe("Limit applied to each entry's free-text description. The 500-character default keeps context from ballooning on members with long donation or directorship write-ups. Only push it to 2000 or more for forensic provenance work."),
        },
        ...
        try {
            ...
            params.set("Take", String(limit));
            if (category)
                params.set("CategoryId", String(INTEREST_CATEGORIES[category]));
            ...
            for (const item of items) {
                ...
                const categoryName = categoryObj && typeof categoryObj === "object" && !Array.isArray(categoryObj)
    ...
    // -------------------------------------------------------------------------
    server.registerTool("parliament_search_petitions", {
...
This is the source for UK Parliament petitions (petition.parliament.uk).`,
        inputSchema: {
            ...
            offset: z
                ...
                .lte(2000)
                .default(0)
                .describe("How many petitions to skip ahead of this page. Defaults to 0. Re-issue the call with offset increased by the number returned as long as has_more is true."),
            limit: z
                .number()
                .int()
                .gte(1)
    ...
    // -------------------------------------------------------------------------
    server.registerTool("parliament_get_debate_divisions", {
...
whenever no match is found.`,
        inputSchema: {
            debate_ext_id: z
                .string()
                .min(8)
                .describe("The debate GUID (DebateSectionExtId). Carry it over from parliament_search_hansard's contribution.debate_ext_id or top_debates[].debate_ext_id, or from parliament_policy_position_summary's top_debates[].debate_ext_id."),
        },
        ...
        try {
            ...
            for (const item of items) {
                ...
                if (parsed !== null)
                    divisions.push(parsed);
            }
            // Match the Hansard-side `id` onto the Lords/Commons Votes API `votes_id`.
            await populateVotesIds(deps, divisions);
            const result = { debate_ext_id, divisions };
            return jsonResult(result);
        }
        catch (err) {
            return toolErrorFromException(err, `parliament_get_debate_divisions(debate_ext_id=${quoteArg(debate_ext_id)})`);
        }
    });
    // -------------------------------------------------------------------------
    // parliament_get_debate_contributions
    // -------------------------------------------------------------------------
    server.registerTool("parliament_get_debate_contributions", {
...
from training data. This is the definitive source for member contributions.`,
        inputSchema: {
            debate_ext_id: z
                .string()
                .min(8)
                .describe("The debate GUID (DebateSectionExtId). Bring it across from parliament_search_hansard's top_debates[].debate_ext_id, parliament_lookup_by_column's matches[].debate_ext_id, or any other tool that yields a debate identifier."),
            member_id: z
                .number()
                .int()
                .gte(1)
                .describe("An optional integer Members API ID. Supply it and only that member's contributions in the debate come back, whatever words they chose. Resolve it through parliament_find_member. Omit it and the whole debate's contributions are returned (a typical debate runs 100-200 items).")
                .optional(),
        },
        ...
        try {
            ...
            for (let idx = 0; idx < items.length; idx++) {
                ...
                if (!itemIsContribution(item))
                    continue;
                if (member_id !== undefined && item.MemberId !== member_id)
                    continue;
            ...
            return toolErrorFromException(err, `parliament_get_debate_contributions(debate_ext_id=${quoteArg(debate_ext_id)})`);
        }
    });
    // -------------------------------------------------------------------------
    // parliament_lookup_by_column
    // -------------------------------------------------------------------------
    server.registerTool("parliament_lookup_by_column", {
...
This is the definitive source for resolving OSCOLA Hansard columns.`,
        inputSchema: {
            column_number: z
                ...
                .min(1)
                .max(20)
                .describe("The Hansard column number taken from an OSCOLA footnote — '200', say, in 'HL Deb 14 Oct 2025, vol 849, col 200'. It is a string rather than an integer so that suffixed columns such as '1162W' for written answers are accepted."),
            volume_number: z
                .number()
                .int()
                .gt(0)
                .describe("The Hansard volume number — the 'vol 849' portion of an OSCOLA citation. It is mandatory: the endpoint resolves a citation only when the volume is supplied, and the sitting date will NOT stand in for it (verified live 2026-05-29)."),
            house: z
                .enum(["Commons", "Lords", "both"])
                .default("both")
        ...
        try {
            ...
            for (const item of results) {
                try {
                    ...
                    try {
                        ...
                        const debatePayload = df.text ? jsonOf(df) : {};
                        const debateItems = debatePayload.Items ?? [];
                        contributionCount = debateItems.reduce((acc, i) => acc + (itemIsContribution(i) ? 1 : 0), 0);
            ...
            return toolErrorFromException(err, `parliament_lookup_by_column(column_number=${quoteArg(column_number)}, volume_number=${volume_number})`);
        }
    });
}
