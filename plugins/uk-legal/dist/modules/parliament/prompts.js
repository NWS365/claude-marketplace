 ...
 */
import { z } from "zod";
export function registerParliamentPrompts(server) {
    server.registerPrompt("parliament_policy_reception_review", {
...
from short snippets — it has the model quote and cite them instead.`,
        ...
        const text = "You are supporting a legal and policy team. The policy under consideration is:\n\n" +
            `${policy_description}\n\n` +
    ...
    server.registerPrompt("parliament_member_record_on_topic", {
...
stance — that reading is left to the caller.`,
        ...
        const text = `Assemble ${member_name}'s parliamentary record on '${topic}' — a citable pack of their ` +
